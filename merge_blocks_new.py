...
d_aryy=[]
def readfile():
    with open("citation_network_metapath_apa+pap/apa+pap_0.5+0.5.txt",encoding='utf-8')as f:
        for line in f:
            ...
            if "similar entities" in line:
                ...
                m_aryy.append(num)
    # m_ary[0] = "<\s>"
    with open("citation_network_metapath_pvp+vpv/pvp+vpv_0.5+0.5.txt",encoding='utf-8')as f:
        for line in f:
            if "target entity" in line:
                num=re.sub(u"([^\u0030-\u0039])","", line)
                #print(num)
    ...
    # print(len(d_ary))
    k=0
    outfile=open("citation_network_apa+pap+pvp+vpv/apa+pap+pvp+vpv_0.5.txt",'w',encoding='utf-8')
