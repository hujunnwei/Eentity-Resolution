...
def average(total_edge,total_weight):
    return total_weight/total_edge

def pruning(average_weight):
    # for i in range(0,len(ary_weight)):
    #     if len(ary_weight[i])==1:
    #         print(ary_weight[i])
    outfile=open("citation_network_1000_first_prune/prune_AM_blocks_(1).txt",'w',encoding='utf-8')
    for i in range(0,len(ary)):
        ...
        for j in range(0,len(aryy[i])):
            if ary_weight[i][0] != '' and float(ary_weight[i][j])>=average_weight:
...
if __name__ == '__main__':
    ...
    average_weight=average(total_edge,total_weight)
    pruning(average_weight)
